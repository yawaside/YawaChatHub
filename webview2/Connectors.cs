...
internal static class Net
{
    ...
    /// поэтому счётчик отставал или застывал. 15 секунд — компромисс между
    /// «живым» счётчиком и лимитами API площадок.
    /// </summary>
    public const int ViewerPollMs = 15_000;
...
public sealed class TwitchIrcConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // Основной источник — официальный Helix (быстрый и точный), запасной —
        // decapi (работает без токена, но часто тормозит и лимитирует запросы).
        // Раньше опрос шёл ТОЛЬКО через decapi раз в 45 секунд, из-за чего
        // счётчик зрителей отставал на минуты или застревал совсем.
        _ = Task.Run(async () =>
        {
            ...
            {
                var updated = false;
                var token = string.IsNullOrWhiteSpace(_botToken) ? ConnectorManager.BotToken : _botToken;
                ...
                if (!string.IsNullOrWhiteSpace(token))
                {
                    try
                    {
                        using var req = new HttpRequestMessage(HttpMethod.Get,
                            $"https://api.twitch.tv/helix/streams?user_login={Uri.EscapeDataString(_user)}");
                ...
                if (!updated)
                {
                    try
                    {
                        var s = (await http.GetStringAsync(
                            $"https://decapi.me/twitch/viewercount/{_user}", ct)).Trim();
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                try
                {
                    var j = await poll.GetStringAsync($"{ApiBase}/blog/{_user}", ct);
                    using var d = JsonDocument.Parse(j);
