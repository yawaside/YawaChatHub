...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    /// в AppDir\runtime; при смене размера (другая версия Node в новой сборке)
    /// файл перезаписывается. Возвращает путь к node.exe или null, если
    /// ресурс в эту сборку не встроен (dev-запуск).
    /// </summary>
    private string? EnsureBundledNode()
    {
        try
        {
            ...
            var target = Path.Combine(RuntimeDir, "node.exe");
            if (!File.Exists(target) || new FileInfo(target).Length != bytes.Length)
            {
                Log("разворачиваем встроенный Node.js (" + bytes.Length / 1024 / 1024 + " МБ)");
