...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    {
        // Каталог моста готовится один раз за запуск приложения: при
        // переподключении канала повторная проверка архива не нужна.
        lock (_runtimeGate)
            if (_cachedBridgeDir != null) return _cachedBridgeDir;

        var ready = FindReadyBridgeDir();
        if (ready != null)
        {
            lock (_runtimeGate) _cachedBridgeDir = ready;
            return ready;
        }
        ...
        if (ExtractBundledBridge(dir))
        {
            lock (_runtimeGate) _cachedBridgeDir = dir;
            return dir;
        }
        ...
        if (!Directory.Exists(Path.Combine(dir, "node_modules"))) return null;
        lock (_runtimeGate) _cachedBridgeDir = dir;
        return dir;
    }
    ...
    /// </summary>
    /* Результат подготовки рантайма на всё время работы приложения.
       РАНЬШЕ при каждом подключении (и каждом переподключении) встроенный
       Node.js — десятки мегабайт — полностью вычитывался в память только
       ради сверки размера файла. Это и давало заметную паузу перед стартом
       чата TikTok. Теперь тяжёлая проверка выполняется один раз. */
    private static string? _cachedNodePath;
    private static string? _cachedBridgeDir;
    private static readonly object _runtimeGate = new();
    ...
    private string? EnsureBundledNode()
    {
        lock (_runtimeGate)
        {
            ...
            try
            {
                ...
                // Размер ресурса известен без чтения содержимого: совпал —
                // файл уже развёрнут, копировать мегабайты незачем.
                if (File.Exists(target) && new FileInfo(target).Length == res.Length)
                    return _cachedNodePath = target;

                Log("разворачиваем встроенный рантайм TikTok (" + res.Length / 1024 / 1024 + " МБ)");
                using (var file = File.Create(target)) res.CopyTo(file);
                return _cachedNodePath = target;
            }
            catch (Exception ex)
            {
    ...
    private bool ExtractBundledBridge(string dir)
    {
        try
        {
            ...
            Directory.CreateDirectory(dir);
            // Сверяем маркер ДО чтения архива: если мост уже развёрнут,
            // распаковка и чтение содержимого не нужны совсем.
            var stamp = res.Length.ToString();
