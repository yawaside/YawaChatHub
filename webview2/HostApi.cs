...
public sealed class HostApi : IDisposable
{
    ...
    /* Облачный голос может быть недоступен (нет сети, сервис закрыт в стране).
       Считаем отказы подряд: после трёх переходим на системный голос на пять
       минут, иначе КАЖДАЯ фраза ждала бы неудачных попыток подключения и
       озвучка «работала только с VPN». Периодически пробуем облако снова. */
    private int _edgeFailures;
    private DateTime _edgeBlockedUntil = DateTime.MinValue;
    private bool EdgeBlocked => DateTime.UtcNow < _edgeBlockedUntil;

    /// Кэш имени русского системного голоса (перебор голосов COM небыстрый).
    private string? _systemRussianVoice;

    /// Запасная озвучка системным голосом Windows — обязательно русским,
    /// иначе Windows читает русский текст английским голосом.
    private Task SpeakWithSystemVoiceAsync(string text, int rate, int volume)
    {
        _systemRussianVoice ??= SapiTts.PickRussianVoice();
        return _tts.SpeakAsync(text, _systemRussianVoice ?? "", rate, volume);
    }
    ...
    public HostApi(MainWindow win, WebView2 web)
    {
        ...
        // TikTok подключается сам (TikTokConnector → tiktok-bridge/bridge.js,
        // модуль tiktok-live-connector) и не требует хуков из HostApi:
        // ни скрытого окна WebView2, ни cookies, ни логина.
        ...
        _connectors = new ConnectorManager(
            ...
            (platform, username, title) =>
                _win.SendEvent("channel.title", new { platform, username, title }));
    }
    ...
    private async Task<object?> Route(string method, object?[] a)
    {
        switch (method)
        {
            ...
            case "tts.speak":
            {
                ...
                {
                    // Облачный голос уже отказал подряд несколько раз (частый
                    // случай — сервис недоступен в стране без VPN). Не тратим
                    // на него время каждой фразы: сразу системный голос.
                    await SpeakWithSystemVoiceAsync(text, rate, volume);
                }
                else
                {
                    try
                    ...
                    {
                        // Облачная озвучка недоступна (сеть, блокировка, отказ
                        // сервиса) — фраза НЕ должна пропадать: озвучиваем
                        // системным голосом Windows как запасной вариант.
                        System.Diagnostics.Debug.WriteLine($"[edge-tts→sapi] {ex.Message}");
                        if (++_edgeFailures >= 3) _edgeBlockedUntil = DateTime.UtcNow.AddMinutes(5);
                        await SpeakWithSystemVoiceAsync(text, rate, volume);
                    }
                }
                return null;
    ...
    /// Булева настройка из settings.json (нужна окну при запуске).
    public bool BoolSetting(string key, bool fallback) =>
        _settings.TryGetPropertyValue(key, out var b) && b is JsonValue bv && bv.TryGetValue<bool>(out var r)
            ? r
            : fallback;
