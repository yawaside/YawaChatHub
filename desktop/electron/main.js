...
ipcMain.on("widget:config", (_e, payload) => {
  if (payload && payload.ttsPlay && widgetServer && widgetServer.sendConfig) {
    const voice = payload.ttsPlay.voice;
    if (edgeTts.isEdgeVoice(voice)) {
      edgeTts.synthesize({
        text: payload.ttsPlay.text,
        voice: voice,
        rate: payload.ttsPlay.rate,
        volume: payload.ttsPlay.volume,
      }).then((result) => {
        if (result) {
          widgetServer.sendConfig({ ttsAudio: { id: payload.ttsPlay.id, audioBase64: result.base64, mime: "audio/mpeg" } });
          // Также проигрываем локально если alsoLocal
          broadcast("sp:edge-audio", { id: payload.ttsPlay.id, base64: result.base64, mime: "audio/mpeg" });
        }
      });
    } else {
      tts.synthesizeWavBase64(payload.ttsPlay).then((audioBase64) => {
        if (audioBase64) widgetServer.sendConfig({ ttsAudio: { id: payload.ttsPlay.id, audioBase64 } });
      });
    }
    return;
  }
...
ipcMain.on("tts:speak", async (_e, payload) => {
  if (payload && edgeTts.isEdgeVoice(payload.voice)) {
    ...
    if (result) {
      // Отправляем MP3 base64 в рендерер — там проигрывается через <audio>
      broadcast("sp:edge-audio", { id: payload.id, base64: result.base64, mime: "audio/mpeg" });
    }
    if (tts.onEnd) tts.onEnd(payload.id);
    return;
  }
  tts.speak(payload);
});
...
ipcMain.handle("tts:voices", async () => {
  const sapi = await tts.voices();
  const edge = edgeTts.voices().map(v => `[Edge] ${v.label}|${v.id}`);
  return [...sapi, ...edge];
});
