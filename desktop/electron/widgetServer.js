...
function makeServer({ token, onClient, widgetFile, chibiFile }) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // Отдельный виджет чиби-модельки — своя страница, не связана с лентой чата
    if (url.pathname === "/chibi") {
      fs.readFile(chibiFile, (err, buf) => {
        if (err) {
          res.writeHead(500);
          res.end("chibi widget not found");
          return;
        }
        res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" });
        res.end(buf);
    ...
    // Статические ассеты виджета (чиби-модельки и прочее)
...
async function startWidgetServer({ port, token, onClient, onPort, onWarn, maxPortTries = 20 }) {
  ...
  const chibiFile = path.join(__dirname, "..", "widget", "chibi.html");
  let state = null;
  let boundPort = port;

  for (let i = 0; i <= maxPortTries; i += 1) {
    const tryPort = port + i;
    const candidate = makeServer({ token, onClient, widgetFile, chibiFile });
    try {
  ...
  return {
    ...
    chibiUrl: `http://127.0.0.1:${boundPort}/chibi?token=${token}`,
    broadcast(msg) {
    ...
    sendConfig(payload) {
      if (!payload) return;
      // копим оформление и настройки: частичный апдейт не должен стирать остальное,
      // иначе виджет при переподключении получит неполную конфигурацию
      if (!payload.ttsPlay && !payload.ttsAudio) {
