...
async function startWidgetServer({ port, token, onClient, onPort, onWarn, maxPortTries = 20 }) {
  ...
  return {
    ...
    /** Мгновенно применяет настройки и стиль в уже открытых OBS-виджетах. */
    sendConfig(payload) {
      if (!payload) return;
      // Долгоживущий конфиг сливается, а не перезаписывается: чат-виджет хранит cfg/look,
      // чиби-виджет хранит tts. Кратковременные ttsPlay/ttsAudio не попадают в snapshot.
      if (!payload.ttsPlay && !payload.ttsAudio) state.config = { ...(state.config || {}), ...payload };
