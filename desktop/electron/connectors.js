...
class ConnectorManager {
  ...
  async _tiktok(entry) {
    ...
    const initialViewers = room?.data?.liveRoom?.liveRoomStats?.userCount;
    client
      .connect()
      .then(() => {
        this._online(entry, `Канал подключён: TikTok Live / @${username}`);
        if (typeof initialViewers === "number") this.setViewers(entry, initialViewers);
      })
      .catch(() => this._offline(entry, 60, `TikTok: @${username} сейчас не в эфире`));

    // онлайн приходит событием roomUser (viewerCount) — без дополнительных запросов
  ...
  async _vk(entry) {
    ...
    try {
      ...
      this._retry(entry, 60000, { note: `VK Play: библиотека чата не загрузилась (${String(e.message || e).slice(0, 40)})` });
      return;
    }
    if (!VKPLMessageClient) {
      this._retry(entry, 60000, { note: "VK Play: библиотека чата недоступна в этой сборке" });
      return;
    }

    // Префлайт: проверяем, что канал существует, до попытки установить WebSocket.
    // Без этого библиотека может уйти в бесконечное ожидание, а UI застревает в «подключение».
    const blogEndpoints = [
    ...
    let blogInfo = null;
    let blogStatus = 0;
    for (const url of blogEndpoints) {
      // eslint-disable-next-line no-await-in-loop
      const r = await request(url, {
        timeout: 10000,
        headers: { referer: `https://live.vkvideo.ru/${channelId}`, accept: "application/json" },
      });
      blogStatus = r.status || blogStatus;
      if (r.ok) {
        try { blogInfo = JSON.parse(r.body); } catch { blogInfo = null; }
        if (blogInfo) break;
      }
    }
    if (!entry.alive) return;
    if (!blogInfo) {
      this._offline(entry, 120, `VK Play: канал ${channelId} не найден (код ${blogStatus}). Введите username без ссылки.`);
      return;
    }
    const initialViewers = findViewerCount(blogInfo);
    if (initialViewers !== undefined) entry.viewers = initialViewers;
    ...
    // Таймаут на client.connect(): если WebSocket-хендшейк с pubsub.live.vkvideo.ru
    // подвисает, UI не должен вечно висеть в «подключение…».
    try {
      const CONNECT_TIMEOUT_MS = 25000;
      await Promise.race([
        client.connect(),
        new Promise((_, reject) => setTimeout(() => reject(new Error("timeout 25s")), CONNECT_TIMEOUT_MS)),
      ]);
      if (!entry.alive) return;
      ...
      if (!entry.alive) return;
      const err = String(e && e.message ? e.message : e).slice(0, 80);
      this._retry(entry, 20000, { note: `VK Play: ${channelId} — ${err}` });
    }
  }
}
