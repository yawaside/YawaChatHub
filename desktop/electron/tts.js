...
class VoiceHost {
  ...
  /* Простой PowerShell-процесс не должен жить вечно: если озвучка молчит
   * 60 секунд — освобождаем его. При следующей фразе _ensure() поднимет новый. */
  _scheduleIdleKill() {
  ...
  _finishCurrent() {
    ...
    if (!this.queue.length) this._scheduleIdleKill();
  }

  _pump() {
    if (this.currentId || !this.queue.length) return;
    this._cancelIdleKill();
  ...
  skip() {
    this._cancelIdleKill();
    if (!this.host) { this._finishCurrent(); return; }
    const child = this.host;
    this.host = null;
    this.ready = false;
    try { child.kill(); } catch { /* noop */ }
...
/**
 * Edge TTS: синтез MP3 в Node (WebSocket) и передача аудио в renderer,
 * где оно играет через обычный <audio>. PowerShell НЕ используется вовсе —
 * раньше на каждую фразу стартовал отдельный процесс MediaPlayer (~1–3 c
 * задержки и заметная нагрузка CPU); теперь задержка = только TTFB синтеза.
 *
 * Семантика очереди как у VoiceHost: в один момент синтезируется/звучит
 * одна фраза; finish(id) вызывается из main, когда renderer сообщил
 * «фраза доиграла» (IPC tts:end) либо сработала страховка по таймауту.
 */
class EdgeVoiceHost {
  ...
  _pump() {
    ...
    synthesizeEdgeToBuffer({
      ...
      if (abortState.aborted || this.currentId !== item.id) return;
      if (!buf || !buf.length) {
        // офлайн/троттлинг — пропускаем фразу, очередь идёт дальше
        this._finishCurrent();
        return;
      }
      // Страховка: если renderer не сообщит конец (окно закрыто и т.п.),
      // двигаем очередь сами. MP3 48 kbit/s ≈ 6000 байт/с.
      const estMs = Math.max(800, Math.round(buf.length / 6));
      this._timer = setTimeout(() => this._finishCurrent(), estMs + 8000);
      if (this.onAudio) this.onAudio(item.id, buf);
      else this._finishCurrent();
    }).catch(() => {
      if (this.currentId === item.id) this._finishCurrent();
    });
  }

  /** Renderer доиграл фразу (или страховка) — освобождаем очередь. */
  finish(id) {
...
class TtsEngine {
  constructor() {
    this.onEnd = null;
    // Колбэк для Edge: готовый MP3 уходит в renderer и играет через <audio>
    // (без PowerShell). signature: (id, buffer) => void
    this.onAudio = null;
  ...
  _edge() {
    if (!this._edgeHost) {
      this._edgeHost = new EdgeVoiceHost(
        (id) => this.onEnd && this.onEnd(id),
        (id, buf) => this.onAudio && this.onAudio(id, buf)
      );
    }
    return this._edgeHost;
  }

  /** Renderer доиграл Edge-фразу — освобождаем очередь синтезов. */
  edgeFinish(id) {
    if (this._edgeHost) this._edgeHost.finish(id);
  }
