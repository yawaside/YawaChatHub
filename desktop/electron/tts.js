...
class TtsEngine {
  ...
  async voices() {
    ...
    const [out64, out32raw, edgeVoices] = await Promise.all([
      process.platform === "win32" ? runPs(psExeFor("64"), LIST_VOICES_SCRIPT) : Promise.resolve(""),
      process.platform === "win32" && fs.existsSync(PS_32) ? runPs(psExeFor("32"), LIST_VOICES_SCRIPT) : Promise.resolve(""),
