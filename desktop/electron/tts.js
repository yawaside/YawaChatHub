...
class EdgeVoiceHost {
  ...
  _playBuffer(buf) {
    ...
    // MediaPlayer умеет MP3; PlaySync у него нет — ждём позицию в цикле.
    // -STA обязателен для WPF-медиакомпонентов.
    const script =
      "$ErrorActionPreference='SilentlyContinue';" +
      ...
      "try{$d=$p.NaturalDuration;if($d -and $d.HasTimeSpan -and $d.TimeSpan.TotalMilliseconds -gt 500){break}}catch{};" +
      "Start-Sleep -Milliseconds 150};" +
      "try{$total=$p.NaturalDuration.TimeSpan.TotalMilliseconds}catch{$total=0};" +
      "if(-not $total -or $total -le 0){$total=8000};" +
      "$t0=Get-Date;" +
      "while(((Get-Date)-$t0).TotalMilliseconds -lt ($total+1500)){" +
      "try{if($p.Clock -eq $null -and $p.Position.TotalMilliseconds -le 0 -and ((Get-Date)-$t0).TotalMilliseconds -gt 2500){break}}catch{};" +
