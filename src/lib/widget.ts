...
/**
 * Ссылка для ОТДЕЛЬНОГО Browser Source с чиби-моделькой.
 * Не связана с виджетом чата: своя страница, свой источник в OBS.
 */
export function buildChibiUrl(port: number, token: string): string {
  return `http://127.0.0.1:${port}/chibi?token=${encodeURIComponent(token)}`;
}
