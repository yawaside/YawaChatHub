...
export function useSpeechEngine() {
  ...
  useEffect(() => {
    if (spTts) {
      spTts.voices().then((names: string[]) => {
        if (!names.length) return;
        const vs = names.map((n: string) => {
          // Edge voices: "[Edge] Label|voiceId"
          const edgeMatch = /^\[Edge\] (.+?)\|(.+)$/.exec(n);
          if (edgeMatch) {
            return {
              voiceURI: edgeMatch[2],
              name: edgeMatch[1],
              lang: "ru-RU",
              default: false,
              localService: false,
          ...
          return { voiceURI: n, name: n, lang: "ru-RU", default: false, localService: true } as SpeechSynthesisVoice;
        });
        setVoices(vs);
        setVoiceURI((cur: string) => cur || vs.find((v: SpeechSynthesisVoice) => /ru|рус|russian|irina|svetlana/i.test(v.name))?.voiceURI || vs[0]?.voiceURI || "");
      });
      return;
    }
    if (!supported) return;
  ...
  const process = useCallback(() => {
    ...
    if (cfg.current.obsTts && typeof window !== "undefined" && (window as any).sp) {
      ...
      // Дополнительно озвучиваем локально через SAPI — слышно в OBS и в ПК одновременно.
      // Длительность локального TTS — реальная, из WAV; оцениваем по символам как fallback.
      if (cfg.current.alsoLocal && spTts && !window.speechSynthesis) {
  ...
  useEffect(() => {
    ...
    // Edge TTS: MP3 base64 проигрывается прямо в окне приложения через <audio>.
    const edgeCb = (spTts as unknown as { onEdgeAudio?: (cb: (data: { id: string; base64: string; mime?: string }) => void) => void }).onEdgeAudio;
    if (edgeCb) {
      edgeCb((data: { id: string; base64: string; mime?: string }) => {
        try {
          const mime = data.mime || "audio/mpeg";
          const audio = new Audio(`data:${mime};base64,${data.base64}`);
          audio.volume = cfg.current.volume;
          audio.play().catch(() => {});
        } catch { /* noop */ }
      });
    }
