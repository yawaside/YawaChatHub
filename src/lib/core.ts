...
export function useSpeechEngine() {
  ...
  const spTts =
    typeof window !== "undefined"
      ? ((window as unknown as { sp?: { tts?: {
          ...
          synthesize: (p: { text: string; rate: number; volume: number; voice?: string }) => Promise<{ audioBase64: string; mime: string } | null>;
          skip: () => void;
          stopAll: () => void;
    ...
    if (spTts) {
      spTts.voices().then((list) => {
        ...
        const vs = list.map(
          (v) =>
            ({
              voiceURI: v.engine === "edge" ? `edge:${v.name}` : v.name,
              name: v.label || v.name,
              lang: "ru-RU",
              default: false,
  ...
  /** Локальное воспроизведение синтезированного аудио (Edge TTS / OBS fallback) через <audio>. */
  const playBridgeAudio = useCallback((audioBase64: string, mime = "audio/wav", onDone?: () => void) => {
    try {
  ...
  const process = useCallback(() => {
    ...
    if (cfg.current.obsTts && typeof window !== "undefined" && (window as any).sp) {
      ...
      // Дополнительно озвучиваем локально — и в OBS, и в этом ПК одновременно.
      if (cfg.current.alsoLocal && spTts) {
        if (cfg.current.voiceURI.startsWith("edge:")) {
          spTts
            .synthesize({
              text: item.text,
              rate: cfg.current.rate,
            ...
            .then((res) => {
              if (res?.audioBase64) playBridgeAudio(res.audioBase64, res.mime);
            })
            .catch(() => {});
        } else {
          try {
      ...
      if (cfg.current.voiceURI.startsWith("edge:")) {
        spTts
          ...
          .then((res) => {
            ...
            playBridgeAudio(res.audioBase64, res.mime, () => {
              speaking.current = false;
              setNow(null);
              window.setTimeout(process, 80);
            });
  ...
  const preview = useCallback(
    async (voice: string, phrase?: string) => {
      ...
      // Desktop bridge
      if (spTts) {
        if (String(voice || "").startsWith("edge:")) {
          const res = await spTts.synthesize({
            text,
            rate: cfg.current.rate,
            volume: cfg.current.volume,
            voice: voice || undefined,
          });
          if (!res?.audioBase64) return { ok: false, error: "Edge TTS недоступен или заблокирован сетью" };
          playBridgeAudio(res.audioBase64, res.mime);
          return { ok: true };
        }
        spTts.speak({
