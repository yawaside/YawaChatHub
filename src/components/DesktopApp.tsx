...
import {
  DEFAULT_CHAT_VIEW, DEFAULT_HOTKEYS, DEFAULT_OVERLAY, DEFAULT_WIDGET, buildWidgetUrl, buildChibiUrl,
} from "../lib/widget";
...
export default function DesktopApp() {
  ...
  /* Настройки модельки уходят в оба виджета сразу при изменении — независимо
     от того, открыта ли вкладка «Виджет OBS». */
  useEffect(() => {
    sp?.widgetConfig?.({ tts: ttsSafe });
  }, [sp, ttsSafe]);

  /* отдельный Browser Source для чиби-модельки */
  const chibiUrl = useMemo(
    () => (widgetInfo.port ? buildChibiUrl(widgetInfo.port, widgetInfo.token) : ""),
    [widgetInfo.port, widgetInfo.token]
  );
