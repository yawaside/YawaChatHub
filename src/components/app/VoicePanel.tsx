...
import { Ban, Copy, Eye, ListX, Mic, MonitorPlay, Pause, Play, SkipForward, Volume2 } from "lucide-react";
...
export default function VoicePanel({ speech, cfg, onChange, toast, chibiUrl }: {
  ...
  toast: (t: string) => void;
  /** ссылка на ОТДЕЛЬНЫЙ Browser Source с моделькой */
  chibiUrl: string;
}) {
  ...
  const copyChibi = async () => {
    if (!chibiUrl) {
      toast("Сервер виджета не запущен — доступно в desktop-версии");
      return;
    }
    try {
      await navigator.clipboard.writeText(chibiUrl);
      toast("Ссылка на модельку скопирована — вставьте в OBS");
    } catch {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Panel
        title="Чиби-моделька в виджете OBS"
        desc="2D-моделька поверх виджета. Рот открывается в такт озвучиваемому сообщению."
        right={
          <Btn variant={cfg.chibiEnabled ? "primary" : "outline"} onClick={() => onChange({ chibiEnabled: !cfg.chibiEnabled })}>
            {cfg.chibiEnabled ? "Включена" : "Выключена"}
          </Btn>
        }
      >
        <div className="grid gap-4 sm:grid-cols-[auto_1fr]">
          ...
          >
            <ChibiSprite
              id={cfg.chibi}
              speaking={cfg.chibiEnabled && !!speech.now}
              className="shrink-0"
              style={{ height: cfg.chibiSize, width: cfg.chibiSize * 0.82 }}
            />
            <span
              className="font-mono text-[9px] uppercase tracking-[0.14em]"
              style={{ color: speech.now && cfg.chibiEnabled ? "#4ade80" : "var(--dw-dim)" }}
            >
              {cfg.chibiEnabled ? (speech.now ? "говорит · липсинк" : "ждёт озвучку") : "выключена"}
            </span>
            <Btn
              variant="ghost"
              disabled={!cfg.enabled}
              onClick={() => { speech.test(); toast("Тестовая фраза"); }}
            >
          ...
          <div className="min-w-0 space-y-3">
            <Toggle
              label="Показывать в виджете OBS"
              hint="моделька рендерится в Browser Source поверх ленты чата"
              on={cfg.chibiEnabled}
                    ...
                    >
                      <span className="grid h-14 place-items-center">
                        {o.id === "none" ? (
                          ...
                          <ChibiSprite id={o.id} speaking={false} style={{ height: 52, width: 44 }} />
                        )}
                      </span>
                      <span
            ...
            {/* отдельный источник в OBS — не связан с виджетом чата */}
            <div className="rounded-xl border p-3" style={{ borderColor: "var(--dw-line)", background: "var(--dw-bg)" }}>
              <div className="flex items-center gap-2">
                <MonitorPlay size={13} style={{ color: "#a78bfa" }} />
                <span className="text-[12px] font-semibold">Отдельный источник в OBS</span>
              </div>
              <p className="mt-1 text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                Моделька — самостоятельный Browser Source: её можно двигать и масштабировать
                независимо от ленты чата. Звук она не дублирует, только анимация.
              </p>
              <div className="mt-2 flex gap-2">
                <input
                  readOnly
                  value={chibiUrl || "сервер виджета не запущен"}
                ...
                />
                <Btn variant="primary" onClick={copyChibi} disabled={!chibiUrl}>
                  <Copy size={11} /> Копировать
                </Btn>
              </div>
              <p className="mt-2 text-[10px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                OBS → Источники → <b>Браузер</b> → вставьте ссылку, размер ≈ {cfg.chibiSize}×{cfg.chibiSize},
                отметьте «Прозрачный фон».
              </p>
            </div>

            <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
              Рот открывается плавно по громкости озвучки. Моргание, покачивание головы, плеч и рук
              работают всегда, пока моделька включена.
            </p>
          </div>
        </div>
      </Panel>
