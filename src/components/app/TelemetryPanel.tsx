    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-2">
      ...
      <Panel title="Предпросмотр" desc="Так телеметрия выглядит поверх игры.">
        ...
        >
          ...
          <div
            className="relative inline-flex max-w-full flex-wrap gap-2 rounded-xl p-2"
            style={{
              // Прозрачность — только подложка и рамка; показатели остаются чёткими.
              background: `rgba(9, 11, 18, ${cfg.opacity / 100})`,
              border: `1px solid rgba(255, 255, 255, ${(0.1 * cfg.opacity) / 100})`,
              boxShadow: `0 8px 30px rgba(0, 0, 0, ${(0.45 * cfg.opacity) / 100})`,
            }}
          >
            {cfg.showCpuLoad && <MetricPreview icon={Cpu} label="CPU" value={formatPercent(stats.cpu.usage)} accent="#60a5fa" alpha={cfg.opacity / 100} fontSize={cfg.fontSize} />}
            {cfg.showCpuTemp && <MetricPreview icon={Thermometer} label="CPU °C" value={formatTemperature(stats.cpu.temperature)} accent="#f97316" alpha={cfg.opacity / 100} fontSize={cfg.fontSize} />}
            {hasCpu && hasGpu && <span className="w-px self-stretch" style={{ background: `rgba(255,255,255,${(0.12 * cfg.opacity) / 100})` }} />}
            {cfg.showGpuLoad && <MetricPreview icon={Gauge} label="GPU" value={formatPercent(stats.gpu.usage)} accent="#a78bfa" alpha={cfg.opacity / 100} fontSize={cfg.fontSize} />}
            {cfg.showGpuTemp && <MetricPreview icon={Thermometer} label="GPU °C" value={formatTemperature(stats.gpu.temperature)} accent="#ef4444" alpha={cfg.opacity / 100} fontSize={cfg.fontSize} />}
            {stats.gpu.name && <span className="self-center font-mono text-[9px] text-white/60"><Video className="mr-1 inline" size={10} />{stats.gpu.name}</span>}
            {!hasCpu && !hasGpu && <span className="px-2 py-1 font-mono text-[10px] text-white/70">Выберите показатели слева</span>}
          </div>
        </div>
