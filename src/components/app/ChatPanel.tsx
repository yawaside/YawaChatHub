...
import {
  Bell, BellOff, ChevronDown, ChevronUp, ChevronsDown, Eye, Plus,
...
function ChannelCard({
  ...
  const viewersTitle = hasViewers
    ? ` · ${fmtViewersFull(c.viewers)} ${pluralViewers(c.viewers as number)}`
    : online
    ? " · онлайн уточняется"
    : "";
  if (compact) {
    return (
      <div className="group relative mx-auto" title={`${PLATFORMS[c.platform].label} · ${c.channelId} · ${st.label}${viewersTitle}`}>
        <span
        ...
        {hasViewers && (
          <span
            className="mt-1 block text-center font-mono text-[9px] leading-none tabular-nums"
        ...
        <span
          className="absolute -right-0.5 top-[30px] h-2.5 w-2.5 rounded-full border-2"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-1.5">
          ...
          {online && (
            <span
              className="ml-auto inline-flex shrink-0 items-center gap-1 rounded-md px-1.5 py-0.5 font-mono text-[10.5px] tabular-nums"
            ...
            >
              <Eye size={10} style={{ color: "var(--dw-dim)" }} />
...
export default function ChatPanel({
  ...
  /* онлайн по подключённым площадкам — для заголовка панели и мобильной сводки */
          ...
          {!channelsCollapsed && <div className="flex items-center gap-2">
            ...
            {viewersTotal !== null && (
              <span
                ...
                title={`Общий онлайн по подключённым площадкам: ${fmtViewersFull(viewersTotal)} ${pluralViewers(viewersTotal)}`}
              >
          ...
          <div className="space-y-[3px]">
            {shown.map((m) =>
              m.sys ? (
                ...
                >
                  {viewCfg.showTime && (
                    <span className="mt-1 shrink-0 font-mono text-[11.5px]" style={{ color: "var(--dw-dim)" }}>
                      {fmtTime(m.ts)}
                    </span>
                  )}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    >
                      <span className="cursor-default font-bold" style={{ color: m.color }}>
                        {m.author}
                      </span>
