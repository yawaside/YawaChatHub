...
import {
  AudioLines, Bot, Cpu, Filter, Gamepad2, HandCoins, Heart, Keyboard, LayoutList, MonitorPlay, Search, X,
} from "lucide-react";
import { usePersisted } from "../../lib/persist";
import type { BotCommand, Channel, ChatViewConfig, DonateConfig, OverlayConfig, TtsConfig, WidgetConfig } from "../../lib/types";
...
export type SettingsTabId =
  | "voice" | "donate" | "filters" | "chatview" | "widget" | "overlay" | "hotkeys" | "system" | "chatbot" | "support"
...
const TABS: { id: SettingsTabId; name: string; icon: React.ReactNode; section: "look" | "app" }[] = [
  { id: "chatview", name: "Лента чата", icon: <LayoutList size={14} />, section: "look" },
  { id: "widget", name: "Виджет OBS", icon: <MonitorPlay size={14} />, section: "look" },
  { id: "overlay", name: "Оверлей поверх игры", icon: <Gamepad2 size={14} />, section: "look" },
  { id: "voice", name: "Озвучка", icon: <AudioLines size={14} />, section: "app" },
  { id: "donate", name: "Донаты", icon: <HandCoins size={14} />, section: "app" },
  { id: "filters", name: "Фильтры чата", icon: <Filter size={14} />, section: "app" },
  { id: "chatbot", name: "Чат-бот", icon: <Bot size={14} />, section: "app" },
  { id: "hotkeys", name: "Горячие клавиши", icon: <Keyboard size={14} />, section: "app" },
  { id: "system", name: "Приложение", icon: <Cpu size={14} />, section: "app" },
  { id: "support", name: "Поддержать", icon: <Heart size={14} />, section: "app" },
];
...
interface Props {
  ...
  donate: DonateConfig;
  patchDonate: (p: Partial<DonateConfig>) => void;
  channels: Channel[];
  onChannelDonateSound: (channelId: string, sound: string) => void;
  queue: QueueItem[];
  paused: boolean;
  onPauseToggle: () => void;
  onSkipQueue: () => void;
  ...
  setStartMaximized: (v: boolean) => void;
  hideDeleted: boolean;
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            {tab === "voice" && (
              <div className="flex flex-col gap-2">
                ...
                {/* раздел «Звук» больше не отдельная вкладка.
                    Сигнал доната и его громкость переехали в раздел «Донаты». */}
                <Collapsible title="Устройство вывода звука">
                  <div className="flex flex-col gap-2">
                    <div>
                    ...
                    <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                      Звук оповещения о донате и озвучка донатов — в разделе «Донаты».
                    </p>
                  </div>
                </Collapsible>
              </div>
            ...
            {tab === "donate" && (
              <DonatePanel
                ...
                patch={p.patchDonate}
                channels={p.channels}
                onChannelSound={p.onChannelDonateSound}
              />
            )}
            ...
            {tab === "chatbot" && (
              <ChatBotPanel
                toast={p.toast}
                commands={p.botCommands}
                onUpdateCommands={p.onUpdateCommands}
              />
            )}
            ...
            {tab === "system" && (
              <div className="flex max-w-[480px] flex-col gap-1">
                <ToggleRow label="Сворачивать окно в трей" checked={p.minimizeToTray} onChange={(v) => { p.setMinimizeToTray(v); p.markSaved(); }} />
                ...
                <ToggleRow label="Запускать свёрнутым в трей" checked={p.startHidden} onChange={(v) => { p.setStartHidden(v); p.markSaved(); }} />
                <ToggleRow
                  label="Разворачивать окно на весь экран при запуске"
                  hint="интерфейс гарантированно виден целиком на любом экране"
                  checked={p.startMaximized}
                  onChange={(v) => { p.setStartMaximized(v); p.markSaved(); }}
                />
