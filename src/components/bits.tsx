...
/* ---------- logo ----------
   Брендинг «Yawa | Chat | Hub»:
   - Yawa — белый/контрастный
   - Chat — градиентный бэкграунд-«стикер» в цветах текущего стиля
   - Hub  — приглушённый акцент
   Шрифт конденсированный, высота ровная, у «стикера» Chat лёгкая тень
   чтобы не выглядеть как текст с подложкой, а как пилл / чип.
*/
export function Logo({ size = 15, accent = "#8b5cf6", accentSoft = "#a78bfa" }: { size?: number; accent?: string; accentSoft?: string }) {
  return (
    <a href="#top" className="group flex items-center gap-2.5">
      <span
        className="grid place-items-center rounded-[10px] transition-transform duration-500 group-hover:rotate-[8deg]"
        style={{
          width: size * 2.3,
          height: size * 2.3,
      ...
      <span className="font-display text-[15px] font-bold leading-none tracking-tight">
        <span className="text-fog">Yawa</span>
        <span
          className="mx-1 inline-block rounded-md px-1.5 py-0.5 align-middle text-[12.5px] tracking-normal"
          style={{
            color: "#fff",
            background: `linear-gradient(135deg, ${accent} 0%, ${accentSoft} 100%)`,
            boxShadow: `0 0 12px ${accent}66, inset 0 0 0 1px rgba(255,255,255,0.18)`,
