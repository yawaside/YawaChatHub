...
**Релизный exe полностью автономен**: Node.js и модули TikTok-моста встроены
внутрь (`npm run bundle:runtime` перед `dotnet publish`, в CI это происходит
само), а озвучка Edge TTS имеет встроенный MP3-декодер на случай ПК без
системных медиакодеков. Ставить на компьютер Node.js/Python/Java не нужно.

Единственный внешний компонент — **Microsoft Edge WebView2 Runtime** (в Windows 11
уже есть). Если его нет или он устарел, приложение при запуске показывает окно
с кнопкой «Установить»: оно само скачивает официальный установщик Microsoft
(≈2 МБ) и ставит компонент — см. `webview2/Prerequisites.cs`.
...
Коннекторы площадок (Twitch IRC, YouTube polling, Kick pusher, VK Live WS,
TikTok через `tiktok-live-connector`, DonationAlerts Centrifugo) подключаются в единственную точку `HostApi.PushChatMessage(...)` —
