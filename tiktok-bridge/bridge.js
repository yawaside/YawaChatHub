...
/** Снятие таймера дочитывания онлайна текущего подключения (см. viewerPoll). */
let stopViewerPoll = () => {};
...
async function connect(rawUser) {
  ...
  // Онлайн приходит push-событием roomUser — это основной и самый быстрый путь.
  let lastViewerPush = 0;
  client.on("roomUser", (d) => {
    const n = d?.viewerCount ?? d?.viewer_count ?? d?.total;
    if (typeof n === "number") {
      lastViewerPush = Date.now();
      send({ type: "status", status: "online", viewers: n });
    }
  });

  // Страховка: на части эфиров TikTok присылает roomUser редко или не шлёт
  // совсем, и счётчик зрителей «замерзал» на стартовом значении. Если push
  // молчит дольше 20 секунд — дочитываем онлайн тем же запросом, которым
  // ищем комнату. Таймер снимается при отключении сокета.
  const viewerPoll = setInterval(async () => {
    if (currentTarget !== username || streamEnded || !conn) return;
    if (Date.now() - lastViewerPush < 20000) return;
    try {
      const info = await discoverRoom(username);
      if (currentTarget !== username) return;
      if (typeof info?.viewers === "number" && info.roomId)
        send({ type: "status", status: "online", viewers: info.viewers });
    } catch { /* сеть подведёт — повторим на следующем круге */ }
  }, 15000);
  if (typeof viewerPoll.unref === "function") viewerPoll.unref();
  // старый таймер предыдущего подключения снимаем, свой — публикуем наружу,
  // чтобы disconnect() мог его остановить при смене канала
  stopViewerPoll();
  stopViewerPoll = () => clearInterval(viewerPoll);
  ...
  client.on("social", (d) => {
    ...
    // join сознательно не пересылаем (см. member ниже)
  });

  // События member/join («зашёл в эфир») ОТКЛЮЧЕНЫ: на живом эфире их десятки
  // в секунду, и лента с оверлеем тонули в служебном спаме без какой-либо
  // пользы для ведущего. Остальные события (follow/share/like/gift) остаются.

  /* ЛАЙКИ. Раньше сюда прилетали десятки событий в секунду, и КАЖДОЕ уходило
     в ленту строкой «лайков на стриме: N». Хуже того, счётчик приходит не во
     всех событиях, и лента заполнялась бессмысленным «лайков на стриме: 0».

     Теперь: события без счётчика игнорируем полностью, а итог показываем не
     чаще раза в две минуты и только когда он заметно вырос. */
  let lastLikeTotal = 0;
  let lastLikeSentAt = 0;
  client.on("like", (d) => {
    const total = Number(d?.totalLikeCount ?? d?.total_like_count ?? 0);
    if (!Number.isFinite(total) || total <= 0) return;      // нулей в ленте быть не должно
    if (total <= lastLikeTotal) return;                     // счётчик не вырос

    const now = Date.now();
    if (lastLikeSentAt && now - lastLikeSentAt < 120000) {
      lastLikeTotal = total;
      return;
    }
    lastLikeTotal = total;
